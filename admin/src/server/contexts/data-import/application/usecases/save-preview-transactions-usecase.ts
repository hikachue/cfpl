  ...
  async execute(input: SavePreviewTransactionsInput): Promise<SavePreviewTransactionsResult> {
    ...
    try {
      ...
      if (insertTransactions.length > 0) {
        // politicalOrganizationId removed from toCreateInput
        // We need to update PreviewTransaction.toCreateInput signature as well?
        // Let's assume we modify PreviewTransaction next or just pass null/undefined here if signature allows.
        // Or better, update PreviewTransaction model.
        // For now, let's remove the argument and we will update PreviewTransaction next.
        const createInputs = insertTransactions.map((transaction) =>
          PreviewTransaction.toCreateInput(transaction),
        );
